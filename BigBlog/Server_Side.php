...
$errors = array();
 ...
 if (isset($_POST['Save'])) {
   ...
   $ville_actuel = mysqli_real_escape_string($db, $_POST['ville_actuel']);
    ...
    // if the user does not choose a image profile then we will insert the default image
   ...
   if (count($errors) == 0) {
    ...
    $username = $_SESSION['username'];
        ...
        if (empty($avatar)) {
          $sql = "INSERT INTO userprofile(profession,phone_number,province,ville_actuel,birthday,avatar,Etat_Civil,Bio,user_id,username) VALUES('$profession','$phone_number','$province','$ville_actuel','$birthday','$full_path','$Etat_Civil','$Bio','$user_id','$username')";
        $query = mysqli_query($db, $sql);
